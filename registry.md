...
**FamilySlug reuse:** the closed `FamilySlug` union (`apr | acr | arr | alr | atr | apur | aer | aar | adr | anr | asr | abr | air | atokr`) is exported once from `@dekimuhq/anchors-envelope/continuity` and reused by `ar.trusted_issuers.v1` (per-issuer `families` arrays) and `ar.issuer_manifest.v1` (`families` field). Single source of truth for the 14-member taxonomy.
